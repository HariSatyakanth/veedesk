...
import 'TaskDetailsPOJO.dart';

class TaskUpDetails extends StatefulWidget {
  final followUpInformation;
  final int screenCheck;

  TaskUpDetails(this.followUpInformation, this.screenCheck);

  @override
  _FollowUpDetailsState createState() => _FollowUpDetailsState();
}

class _FollowUpDetailsState extends State<TaskUpDetails> {
...
//                _response["description"]==null?"Please wait...":_response["description"],
  ...
  Future<dynamic> hitUpdateService() async {
    ...
    try {
      await http
          .get(
              "${Constants.BaseUrl}action=followupTaskUpdate&id=${followUp.id}&Status=1")
          .then((res) => setState(() {

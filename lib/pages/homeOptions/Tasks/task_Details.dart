...
import 'package:newproject/pages/homeOptions/Tasks/TaskDetailsPOJO.dart';
...
class TaskDetails extends StatefulWidget {
  ...
  TaskDetails(this.taskId, this.screenCheck);

  @override
  _TaskDetailsState createState() => _TaskDetailsState(taskId);
}

class _TaskDetailsState extends State<TaskDetails> implements WidgetCallBack {
  String taskId;

  _TaskDetailsState(this.taskId);

  TasksDetailPOJO _tasksDetailPOJO = TasksDetailPOJO();
                  ...
                  children: [
                            ...
                            children: [
                              InkWell(
                                onTap: () {
                                  isPlaying
                                      ? pause()
                                      : play(_tasksDetailPOJO.audioUpload);
                                },
                                child: CircleAvatar(
                                  radius: 15.0,
                    ...
                    Row(
                      children: [
                        ...
                        SizedBox(width: 20.0),
                        showImage(context, listImage, _tasksDetailPOJO.task),
                      ],
                    ),
                    SizedBox(height: 20.0),
                    //  _images(),
                    ...
                    widget.screenCheck == 1
                        ? Row(
                            ...
                            children: [
                              SizedBox(
                                width:
                                    Constants().containerWidth(context) * 0.4,
                                height: 35,
                                child: TextField(
                                  ...
                                  onSubmitted: (value) {
                                    if (progessValue <= int.parse(value) &&
                                        int.parse(value) <= 100 ) {

                                      changeProgress(value);
                                    } else {
                                      Constants.showToast(
                                          "Enter valid progress between ${progessValue} to 100.");
                                    }
                                  },
                                ),
                              ),
                    ...
                    _tasksDetailPOJO.followupTask == null
                        ? Text(
                    ...
                    _tasksDetailPOJO.followupTask != null
                        ? ListView.builder(
                            ...
                            itemCount: _tasksDetailPOJO.followupTask.length,
                              ...
                              return Row(
                                children: [
                                  ...
                                  Expanded(
                                      child: InkWell(
                                    onTap: () {
                                      if (_tasksDetailPOJO.followupTask !=
                                          null) {
                                        Navigator.push(
                                            context,
                                            MaterialPageRoute(
                                                  ...
                                                  Row(
                                                    children: [
                                                      ...
                                                      Row(
                                                        ...
                                                        children: [
                                                          Text(
                                                            Util.changeDateToFormetted( _tasksDetailPOJO.followupTask[index]
                                                                .followupTime),
//                                              _response["description"]==null?"Please wait...":_response["description"],
                                                            style: Constants()
                                                                .txtStyleFont16(
  ...
  _radioValuesFollowUP(int i) {
    switch (int.parse(_tasksDetailPOJO.followupTask[i].status)) {
      case 0:
      ...
      child: ClipRRect(
        ...
        child: AppBar(
          ...
          title: _loading
              ...
              : Text(
                  _tasksDetailPOJO.task == null
                      ? "Please wait..."
                      : _tasksDetailPOJO.task,
                  style:
  ...
  _images() {
    return Row(
      children: [
          ...
          children: [
            CircleAvatar(
              ...
              backgroundImage: NetworkImage(
                _tasksDetailPOJO.asingImage == null
                    ? Constants.USER_IMAGE
                    : _tasksDetailPOJO.asingImage,
              ),
            ),
            Align(
        ...
        SizedBox(width: 5.0),
        Expanded(
            child: Text(_tasksDetailPOJO.asingName,
      ...
      child: DropdownButton(
          ...
          onChanged: (value) {
            if(widget.screenCheck == 1) {
              ...
              Constants.showToast("Only assigned user will updated the task status");
            }
          }),
    );
  }
  ...
  Future<dynamic> _getTaskStatus() async {
    try {
      await http.get("${Constants.BaseUrl}action=getStatusTask").then((res) {
        print(res.body);
        ...
        if (statusCode == 200) {
          ...
          if (_response["success"] == "1") {
            print("success = ${_response["status"][0]["Task_Status"]}");
            for (int i = 0; i < _response["status"].length; i++) {
              setState(() {
                _status.add(_response["status"][i]["Task_Status"]);
              });
            }
          } else {
...
//          Scaffold.of(context).showSnackBar(Constants().showSnackMessage(response["Something went wrong !"]));
  ...
  Future<dynamic> _getTaskDetails() async {
    print("${Constants.BaseUrl}action=taskDetails&id=$taskId");
    try {
      await http
          .get("${Constants.BaseUrl}action=taskDetails&id=$taskId")
          .then((res) {
        print("status : ${res.body}");
        setState(() {
          _response = json.decode(res.body);
        ...
        if (statusCode == 200) {
          print("check 200${_response["success"]}");
          if (_response["success"] == 1) {
            _tasksDetailPOJO = TasksDetailPOJO.fromJson(_response);
            setState(() {
              taskList = _tasksDetailPOJO.task;
              _selectedStatus = _tasksDetailPOJO.currentStatus;
              listImage.clear();
              listImage = Constants.getImageArray(_tasksDetailPOJO.imageUpload);
              progressCtrl.text=_tasksDetailPOJO.progress;
              progessValue=int.parse(_tasksDetailPOJO.progress);
            });
...
//          Scaffold.of(context).showSnackBar(Constants().showSnackMessage(response["Something went wrong !"]));
  ...
  Future<dynamic> changeLeadsStatus() async {
    print(
        "${Constants.BaseUrl}action=changeStatusTask&id=$taskId&status=$statusID");
    _loading = true;
...
//          Scaffold.of(context).showSnackBar(Constants().showSnackMessage(response["Something went wrong !"]));
  ...
  //change Progress
  Future<dynamic> changeProgress(String progress) async {
    print(
        "${Constants.BaseUrl}action=updateTask&id=$taskId&progress=$progress");
    _loading = true;
    try {
      await http
          .get(
        ...
        print(statusCode);
        if (statusCode == 200) {
          Constants.showToast("Progress Updated!");
          setState(() {
            _loading=false;
            progessValue=int.parse(progress);
          });
        } else {
          setState(() {
            _loading=false;

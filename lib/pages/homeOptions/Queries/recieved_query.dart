...
import 'package:newproject/pages/homeOptions/Queries/mine_query.dart';
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Colors.grey[200],
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: (){
          Navigator.push(context, MaterialPageRoute(builder: (context) => AddNewQuery()));
        },
      ...
      child: ClipRRect(
        ...
        child: AppBar(backgroundColor: ColorsUsed.baseColor,
          ...
          actions: [
           InkWell(
             onTap: (){
               Navigator.push(context,MaterialPageRoute(builder: (context)=> MineQuery()));
             },
             child:  Image.asset(
      ...
      return ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return GestureDetector(
              onTap: ()  {
//                Navigator.push(context, MaterialPageRoute(builder: (context) => QueryDetails(),));
                Navigator.of(context).push(MaterialPageRoute(
                    builder: (context)=>QueryDetails(activeTaskList[index].id,1))).then((value) {
                  if(value == true){
                    setState(() {
                      _loading = true;
                    });_getActiveTaskLists();
...
//                Navigator.push(context, MaterialPageRoute(builder: (context) => QueryDetails(),));
                Navigator.of(context).push(MaterialPageRoute(
                    builder: (context)=>QueryDetails(completedTaskList[index].id,2))).then((value) {
                  if(value == true){
                    setState(() {
                      _loading = true;
                    });_getActiveTaskLists();
...
//get Task lists
 var responseList;
  Future<dynamic> _getActiveTaskLists() async{
    ...
    try{
      await http.get("${Constants.BaseUrl}action=getQueriesToList&query_to=${Constants.userId}").then((res){
        print(res.body);
        print("status${res.body}");
        setState(() {
          _loading = false;
          _response = json.decode(res.body);
         responseList = _response["getQueriesToList"];
              ...
              if( status == "1"){
                setState(() {
                  activeTaskList.add(GetQueriesByList(
                    ...
                    totalFollowup: responseList[i]["total_followup"],

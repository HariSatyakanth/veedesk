...
class AddNewQuery extends StatefulWidget {
  final LocalFileSystem localFileSystem;

  AddNewQuery({localFileSystem})
      : this.localFileSystem = localFileSystem ?? LocalFileSystem();
  @override
  AddNewQueryState createState() => AddNewQueryState();
}

class AddNewQueryState extends State<AddNewQuery> with SingleTickerProviderStateMixin implements WidgetCallBack,ImageCallBack {
  double _progess = 0;
  String roleId,empId;
  File _image;
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Constants.commonEditTextFieldWithoutIcon( _taskController, false,
                                "Query Title", "", TextInputType.multiline),
                          ),
                        ],
                      ),
...
//                      Text("timer: 00:"+audioTimer.toString(),style: Constants().txtStyleFont16(Colors.black, 13.0),),
  ...
  Widget showImage(BuildContext context, String name) {
    return images.length > 0
        ? Util.listOfImages(context: context,images: images,name: "Query",widgetCallBack: this)
        : new Container(
      height: 0.0,
    );
  }
...
//    startHandler(true);
  ...
  Future<dynamic> addTask(BuildContext context) async{
    ...
    print("${Constants.BaseUrl}action=queriesInsert&title=${_taskController.text}&"
        "query_to=${empId}&status=1&sevirity=1&"
        "query_by=${Constants.userId}&queryDateTime=$dateToday&description=${_descController.text}&"
        "imagesUrl=$imageArrayList&audioUrl=$audioArrayList");
    setState(() {
      _loader= true;
    });
    try{
      await http.get("${Constants.BaseUrl}action=queriesInsert&title=${_taskController.text}&"
          "query_to=${empId}&status=1&sevirity=1&"
          "query_by=${Constants.userId}&queryDateTime=$dateToday&description=${_descController.text}&"
          "imagesUrl=$imageArrayList&audioUrl=$audioArrayList").then(( res) {
        print(res.body);
        var response = json.decode(res.body);
        final int statusCode = res.statusCode;
        print(statusCode);

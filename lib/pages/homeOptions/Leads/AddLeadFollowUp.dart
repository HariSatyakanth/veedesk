...
class LeadFollowUpAdd extends StatefulWidget {

  final taskId;
  final LocalFileSystem localFileSystem;
  //const FollowUpAdd({this.taskId});
  LeadFollowUpAdd({localFileSystem,this.taskId}) : this.localFileSystem = localFileSystem ?? LocalFileSystem();

  @override
  _FollowUpActionState createState() => _FollowUpActionState();
}

class _FollowUpActionState extends State<LeadFollowUpAdd> with SingleTickerProviderStateMixin
  ...
  List imageArrayList = [];
  File _image;
  String _imgURL = "",_audioFileName="",_audioUrl = "";
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        Row(
                          ...
                          children: [
                            Expanded(child: showImage(context, "My Lead")),
                          ],
                        ),
                        Row(
...
//          prefixText: "$_prefixText  "
  ...
  Future<dynamic> _addFollowUp() async {
    //https://tasla.app/MyOffice/index.php?
    // action=followupLeadsInsert&Project_Id=8&Followup_Date=2020-12-16
    // &Followup_Text=kjjjkk&Attachment=kk&Status=1&Followup_By=2&Description=hjh
    print("${Constants.BaseUrl}action=followupLeadsInsert&Followup_By=${Constants
        .userId}&Project_Id=${widget.taskId}&"
        "Followup_Text=${_titleCtrl.text}&Followup_Date=${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}"
        "&Description=${_descriptionCtrl
        .text}&Attachment=$imageArrayList&Status=0&audioUrl="+_audioUrl);
    try {
      await http.get(
          "${Constants.BaseUrl}action=followupLeadsInsert&Followup_By=${Constants
          .userId}&Project_Id=${widget.taskId}&"
      "Followup_Text=${_titleCtrl.text}&Followup_Date=${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}"
    "&Description=${_descriptionCtrl
        .text}&Attachment=$imageArrayList&Status=0&audioUrl="+_audioUrl).then((res) {
        print(res.body);
